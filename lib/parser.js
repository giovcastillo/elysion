  ...
  parse(opts = { registry: {} }) {
    ...
    switch (rule) {
      ...
      case "INDEX_START Expression INDEX_END": {
        output += "[" + Access.parse({ ...opts, isValue: true }) + "]";
        break;
      }
    }
    return output;
...
Nodes.Array = class Array extends Base {
  parse({ that, $such, scope, addSemicolon, vars, varExistent, constants, prevLine, isCondition, isValue, tabs, wrap, isClass, isAssignment, ID, comments, lastNodeLocation, registry = {}, isFrom, isObjProperty }) {
    ...
    for (let comment of comments.filter(c => (c.loc.first_line < initLoc.first_line) || (c.loc.first_line === initLoc.first_line && c.loc.first_column < initLoc.first_column))) {
      ...
      let commentOutput = parseComment(comment, tabs + 1, true);
    ...
    for (let Spot of Arr) {
      ...
      if (matchedComments.length) {
        for (let comment of matchedComments) {
          ...
          cO = parseComment(comment, tabs + 1, true);
...
Nodes.Assign = class Assign extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, constant, construct, isValue, prevLine, tabs, lineReturns, isCondition, isCompare, comments = [], lastNodeLocation, registry = {}, ID = [], FiresSuper, FiresAwait, FiresYield, isStatement, addSemicolon, afterParse, metaComments } = {}) {
    ...
    output += Assign.parse({ that, $such, scope, vars, varExistent, constants, constant, construct, isValue, prevLine, tabs, lineReturns, isCondition, isCompare, comments, lastNodeLocation, registry, ID, FiresSuper, FiresAwait, FiresYield, isStatement, addSemicolon, afterParse, metaComments });
    ...
    switch (rule) {
      case "Identifier": {
        ...
        if (isAssignment) {
          ...
          if (!metaType) {
            metaType = getDataOfMetaComments({ metaComments, ID: vname, tabs, loc: this.loc });
          } else {
            attachDescriptions(metaComments, [metaType], tabs);
          }
      ...
      case "Value Access": {
        ...
        _refer += Assignable[1].parse({ that, $such, scope, addSemicolon: [], vars, varExistent, constants, prevLine, isCondition, isValue: true, tabs, wrap, isClass, isLine, lineReturns, func, scopedParams, comments, lastNodeLocation, registry: { ...registry, sources: undefined }, FiresSoak, isAssignment, isInvoked: true, isOperation, isParam, isNarrow });

        let s = (registry.sources ? registry.sources.add(this.loc) : '') + _refer;
        ...
        if (isAssignment) {
          if (Assignable[1].rule === "Assignable" && /Identifier|Value Access/.test(Assignable[1].unwrap.rule)) {
            _parent = _refer;
          }
...
Nodes.Assignment = class Assignment extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, constant, construct, isValue, prevLine, tabs, lineReturns, isCondition, isCompare, comments, lastNodeLocation, registry = {}, ID = [], FiresSuper, FiresAwait, FiresYield, isStatement, addSemicolon, afterParse, metaComments }) {
    ...
    if (!["Value Access", "ThisProperty"].includes(Assign[1].unwrap.rule)) {
      if (!vars.__data) {
        vars.__data = [];
      }
      ...
      let mData = addMetaDataToPrevLine({
        prevLine, typeData, tabs, metaComments
      });
...
Nodes.Body = class Body extends Base {
  parse({ tabs = 0, vars = [], constants = [], that = [], $such = [], scope = [], isChildren = false, isClass = false, callSelf = false, func = false, scopedParams = Object.assign([], { generated: true }), top = 0, lastValue = [], varExistent = [], comments = [], lastNodeLocation = { first_line: 1, first_column: 1, last_line: 1, last_column: 1 }, registry = {}, debug = false, FiresSuper = [], FiresYield = [], FiresAwait = [], assignRes, appendToVars, isParenthetical = false } = {}) {
    ...
    for (let Line of Lines) {
      ...
      if ((count === Lines.length) && (assignRes || callSelf || func)) {
      ...
      if (matchedComments.length) {
        for (let comment of matchedComments) {
          let commentOutput = parseComment(comment, tabs);
    ...
    if (!isChildren && !isClass) {
      ...
      if (vars.length) {
        ...
        if (dTypes.length && !registry.omitTypeScript) {
          ...
          ddef += buildVarDefinitionsWithJSDoc(dTypes, tabs, registry.useVar ? "var " : "let ", comments.filter(c => c.jsdoc));
          _vars = _vars.filter(v => !dTypes.find(d => d.ID === v));
        }

        output = (_vars.length ? tab(tabs) + (registry.useVar ? "var " : "let ") + _vars.join(', ') + ";\n\n" : '') + ddef + output;
      }
    } else if (appendToVars) {
      output = tab(tabs) + (registry.useVar ? "var " : "let ") + appendToVars + ';\n\n' + output;
    }
...
Nodes.Class = class Class extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, prevLine, isValue, tabs = 0, lineReturns, addSemicolon, comments = [], lastNodeLocation, registry = {} } = {}) {
    ...
    if (Class[0]) {
      ...
      puppet = true;
      className = Reference(scope, true);
      $such[0] = className;
    }
    ...
    let Body, ForeignExps; // a copy

    Body = Class[2];

    if (Body instanceof Nodes.Body) {
      ForeignExps = [];
      for (var _i = 0, _len = Body[1].length; _i < _len; _i++) {
        var Line = Body[1][_i];
        if ((Line.unwrap.rule === "Value") && !!Line.unwrapUntil(node => (node instanceof Nodes.Function) && !/FUNCTION/.test(node.rule))) {
          continue;
        }
    ...
    if (!ForeignExps.length) {
      if (Body instanceof Nodes.Body) {
        Body = new Nodes.Body(null, Body.unwrap.map(Line => Object.assign(Line, { lineCount: 1 }))).parse({ that, $such, scope, tabs: tabs + 1, isClass: { doesExtend: !!classExtends }, isChildren: true, vars, varExistent, constants, comments, lastNodeLocation: (Class[1] || Class[0] || { loc: lastNodeLocation }).loc, debug: true, registry });
      } else {
        Body = ""
      }
      output += header;
      ...
      var lastNode;
      Body = Body.unwrap.filter((Line) => {
        let Exp = Line.unwrap;
        if ((Exp.rule == "Value") && ['Function'].includes(Exp.unwrap.rule) && !/FUNCTION/.test(Exp.unwrap.unwrap.rule)) {
          lastNode = Line.loc;
          return true;
        }
        return false;
      ...
      let puppetVars = [];
      let parsedBody = "";
      parsedBody += tab(tabs + 1) + new Nodes.Class(null, [className], Class[1], new Nodes.Body(null, Body)).parse({ that, $such, scope, tabs: tabs + 1, isChildren: true, isClass: { doesExtend: !!classExtends }, comments, lastNodeLocation: (Class[1] || Class[0]).loc, registry });
      parsedBody += "\n\n" + tab(tabs + 1) + new Nodes.Body(null, ForeignExps).parse({ that: [className], scope, tabs: tabs + 1, isChildren: true, vars: puppetVars, varExistent, constants, comments, lastNodeLocation: lastNode, registry });
      parsedBody += "\n\n" + tab(tabs + 1) + "return " + className + ";";

      if (puppetVars.length) {
        parsedBody = tab(tabs + 1) + (registry.useVar ? "var " : "let ") + puppetVars.join(', ') + ";\n\n" + parsedBody;
      }

      output = className + " = (function() {\n" + parsedBody + "\n" + tab(tabs) + "}).call(this)";
...
Nodes.Expression = class Expression extends Base {
  parse({ lineReturns = false, addSemicolon = [], construct = false, constants = [], vars = [], varExistent = [], that = [], $such = [], scope = [], isValue = false, isInvoked = false, isOperation = false, isLine = false, prevLine = [], tabs = 0, isCondition = false, isDirect = false, isClass = false, isCompare = false, isAssignment = false, isAssigned = false, ID = [], isParam = false, nl, func = false, scopedParams = Object.assign([], { generated: true }), comments = [], afterParse, metaComments = [], lastNodeLocation = { first_line: 0, first_column: 0, last_line: 0, last_column: 0 }, registry = {}, isUnless = false, FiresSuper = [], FiresAwait = [], FiresYield = [], assignRes = false, isStatement = false, constant = false, FiresSoak, isNarrow = false, isObjProperty, metaQueue } = {}) {
    ...
    switch (this.rule) {
      ...
      case "Value": {
        if (!registry.isELSON && (isInvoked || isLine) && !lineReturns && (['Assignable'].includes(Exp.rule) && !["Identifier", "Array", "Value Access"].includes(Exp[1].rule))) {
          wrap = true;
        }
        ...
        output += Exp.parse({ lineReturns, addSemicolon, construct, that, $such, scope, constants, vars, varExistent, isValue, prevLine, tabs, isCondition, isOperation, isStatement, isDirect, isClass, isLine, isAssignment, ID, isParam, func, scopedParams, isAssigned, comments, lastNodeLocation, registry, FiresSuper, FiresAwait, FiresYield, FiresSoak, isNarrow, isObjProperty, afterParse, metaComments, metaQueue });
        break;
      };
      case "Class": {
      ...
      case "Assign": {
        ...
        output += Exp.parse({ vars, varExistent, lineReturns, that, $such, scope, constants, construct, isValue, prevLine, isCondition, isCompare, tabs, func, scopedParams, comments, lastNodeLocation, registry, FiresSuper, FiresAwait, FiresYield, ID, isStatement, constant, addSemicolon, afterParse, metaComments });
        break;
      };
      case "If": {
        if (isInvoked) wrap = true;
      ...
      case "Code": {
        ...
        if (isInline && Arrow === "=>") {
          ...
          output += "{\n" + tab(tabs + 1) + Body.parse({ lineReturns: true, constants, that, $such, scope: scope.concat(...Params), varExistent: vars, varExistent, tabs: tabs + 1, func: true, scopedParams: Params, comments, lastNodeLocation, registry, FiresAwait: Awaits, FiresYield: Generator }) + "\n" + tab(tabs) + "}";
        }
      ...
      case "Label": {
        ...
        output += `{\n${tab(tabs + 1)}${Bd.parse({ that, $such, scope, vars, varExistent, constants, tabs: tabs + 1, comments, registry, isChildren: true, assignRes, prevLine, FiresSuper, FiresAwait, FiresYield })}\n${tab(tabs)}}`
        break;
      }
    }
    ...
    if (typeof Header[1] === "string") {
      if (/for_(from|at|as)/.test(Header[1])) {
        ...
        if (Kwd === "for_of" || (Kwd === "for_in" && Assignable.rule === "Identifier")) {
          ...
          output += `for (${_i} in (${_ref} = ${TTarget.parse({ that, $such, prevLine, scope, vars, varExistent, constants, isAssigned: true, isValue: true, addSemicolon: [], registry, comments, FiresSuper, FiresAwait, FiresYield })})) {\n${tab(tabs)}${Assignments.length ? Assignments.join(`;\n${tab(tabs)}`) + `;\n${tab(tabs)}` : ''}${section}${Body.parse({ prevLine, lineReturns, that, $such, scope, vars, varExistent, constants, tabs, comments, registry, isChildren: true, assignRes, FiresSuper, FiresAwait, FiresYield })}\n${tab(tabs - 1)}}`
        }
      }
    } else {
...
Nodes.Function = class Function extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, prevLine, isValue, tabs, func, scopedParams, comments, lastNodeLocation, registry = {}, FiresSuper, isInvoked, isParam, isClass, isObject, lineReturns, isExport, isLine, metaComments = [], metaQueue }) {
    ...
    if (childComments.length && Value[1].loc) {
      for (let comment of childComments) {
        ...
        if (comment.loc.first_line === Value[1].loc.first_line) {
          commentOutput += parseComment(comment, tabs) + tab(tabs + 1);
          registry[comment.id] = true;
        }
      }
    }
    ...
    let Awaits = [], Yields = [];
    output = Value[1].parse({ scope: scope.concat(...Params), isValue, tabs: tabs + 1, lineReturns, func: (FuncName === "constructor") && isClass ? false : true, scopedParams: Params, varExistent: [...vars, ...varExistent], comments, lastNodeLocation, registry, FiresAwait: Awaits, FiresYield: Yields });
    ...
    if (returns && returns.length && !registry.omitTypeScript) {
      registry.isTypeScript = true;
      header += `: ${returns[0].nodes.parse({ registry })}`
    }
...
Nodes.Invocation = class Invocation extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, prevLine, isValue, tabs = 0, lineReturns, func, scopedParams, comments = [], lastNodeLocation, registry = {}, FiresSuper = [], FiresSoak } = {}) {
      ...
      Params = Params.contents.map((Param, i, r) => {
        ...
        if (matchedComments.length) {
          for (let comment of matchedComments) {
            ...
            cO = separator + parseComment(comment, tabs + 1, true);
            res = `${cO}${res}`
            registry[comment.id] = true;
          }
    ...
    for (let comment of comments.filter(c => c.loc.first_line >= lastNode.last_line)) {
      if (registry[comment.id]) continue;
      x = true;
      let commentOutput = parseComment(comment, tabs, true);
      if (comment.loc.first_line == lastNode.last_line) {
        let resource = AfterComments, trim = 0;
        while (/\n|\r| /.test(resource.charAt(resource.length - 1))) {
          resource = resource.slice(0, -1);
    ...
    if (x) ParamsOutput = "\n" + tab(tabs + 1) + BeforeComments + ParamsOutput + AfterComments + "\n" + tab(tabs);
...
Nodes.Line = class Line extends Base {
  parse({ that, $such, scope, constants, vars, varExistent, tabs = 0, lineReturns, isClass, isLine = true, func, scopedParams, nl, lastValue, comments = [], afterParse, metaComments = [], lastNodeLocation, registry = {}, FiresSuper = [], FiresYield = [], FiresAwait = [], assignRes = false, isParenthetical } = {}) {
    ...
    let output = "";
    var prevLine = [];
    output += tab(tabs) + (!!Expression.parse ? Expression.parse({ addSemicolon: (registry.isELSON || isParenthetical) ? [] : [1], that, $such, scope, constants, vars, varExistent, prevLine, tabs, lineReturns, isClass, isLine, func, scopedParams, nl, comments, lastNodeLocation, registry, FiresSuper, FiresYield, FiresAwait, assignRes, isParenthetical, isValue: !!isParenthetical, afterParse, metaComments }) : Expression);

    if (prevLine.length) output = tab(tabs) + prevLine.join(colon + tab(tabs)) + colon + output;
    return output
  }
}
  ...
  parse({ that, $such, scope, addSemicolon = [], vars, varExistent, constants, prevLine, isCondition, isValue, isParam, tabs = 0, lineReturns, wrap, isClass, isAssignment, isStatement, ID = [], comments = [], lastNodeLocation, registry = {}, typeData = [], afterParse, metaComments = [] } = {}) {
    ...
    for (let Obj of Object) {
      ...
      if (matchedComments.length) {
        for (let comment of matchedComments) {
          if (registry[comment.id]) continue;
          cO = parseComment(comment, tabs, true).trim();
      ...
      if (metaQueue.length) {
        for (let metaComment of metaQueue) {
          ...
          let mc = '\n' + tab(tabs + 1) + parseComment(metaComment, tabs);
      ...
      if (Id !== undefined) {
        if (!typeMatch) {
          typeMatch = getDataOfMetaComments({ metaComments, ID: Id, tabs, loc: this.loc });
        } else {
          attachDescriptions(metaComments, [typeMatch], tabs);
        }
    ...
    if (Assignments.length) {
      ...
      if (x) output += `\n${tab(tabs + 1)}${(BeforeComments + Assignments + AfterComments).trim()}\n${tab(tabs)}`
    ...
    switch (this.rule) {
      ...
      case "Export": {
        ...
        if (declarations) {
          ...
          list;
          output += "export {" + `\n${tab(tabs + 1)}${list.unwrap.contents.map(l => {
            if (l[1]) {
...
Nodes.Switch = class Switch extends Base {
  parse({ that, $such, assignRes, lineReturns, scope, vars, varExistent, constants, registry = {}, FiresSuper, prevLine, tabs }) {
    ...
    for (let [rule, cases, Body] of Cases.contents) {
      ...
      output += ` {\n${tab(tabs + 1)}` + Body.parse({ that, $such: hasCondition ? [refVar] : $such, assignRes, lineReturns, scope, vars, varExistent, constants, registry, FiresSuper, prevLine, tabs: tabs + 1, isChildren: true }) + (Body.rule === null ? `\n${tab(tabs + 1)}break;` : '') + `\n${tab(tabs)}}`;
    ...
    switch (rule) {
      ...
      case 'Parenthetical': {
        ...
        if (Value instanceof Nodes.Expression) {
          output += Value.parse({ vars, varExistent, that, $such, scope, constants, prevLine, isValue, isCompare, tabs, isCondition, inverse, addSemicolon: [], comments, lastNodeLocation, registry, FiresSoak, isInvoked });

          if (isCondition && isDirect) wrap = false;
          else isValue = true;
        } else {
          output += Value.parse({ vars, varExistent, that, $such, scope, constants, prevLine, isValue, isCompare, tabs, isCondition, inverse, addSemicolon: [], comments, lastNodeLocation, registry, isChildren: true, isParenthetical: true, FiresSoak, isInvoked });
          if (isCondition && isDirect) wrap = false;
          else isValue = true;
        }
        break;
      ...
      case "Invocation": {
        output += Value.parse({ that, $such, scope, vars, varExistent, constants, prevLine, isValue, tabs, func, scopedParams, comments, lastNodeLocation, registry, FiresSuper, FiresSoak, isInvoked, isParam, isObjProperty });
        break;
      };
      case "Function": {
        addSemicolon.pop();
...
function parseComment(comment, tabs, forceWrap = false) {
  let commentOutput = "";
  if (!forceWrap && comment.inline) commentOutput += "// " + comment[1].trim();
  else commentOutput += "/*" + comment[1].split(/\n/g).map((c, i) => (i ? tab(tabs) : "") + c).join("\n") + "*/";
  if (!/\n +\*\/$/.test(commentOutput) && forceWrap) {
    if (!/ \*\/$/.test(commentOutput)) {
      commentOutput = commentOutput.slice(0, -2) + " */"
    }
  }
  let after = forceWrap ? tab(tabs) : "";
  commentOutput += comment.addNewlines ? "\n".repeat(comment.addNewlines) + after : forceWrap ? " " : ""
  return commentOutput;
}

function ParseParam(Parameter, { scope, vars, tabs, constants, $such, that, varExistent, Insert, Params, comments = [], lastNodeLocation, registry = {} }) {
  let output = "";
  comments = [];
  switch (Parameter[0]) {
    case "ParamArray": {
      return ParseParamArray(Parameter[1].unwrap, { scope, vars, varExistent, tabs, constants, $such, that, varExistent, Insert, Params, comments, comments, lastNodeLocation, registry });
    };
    case "ParamObject": {
      return ParseParamObject(Parameter[1].unwrap, { scope, vars, varExistent, tabs, constants, $such, that, varExistent, Insert, Params, comments, comments, lastNodeLocation, registry, type: !registry.omitTypeScript && Parameter[1].loc.type });
    };
    case "ParamIdentifier": {
      ...
      if (This) {
        ...
        Insert.push([Name, refName]); // [thisProperty, paramName]
        Params.push(refName);
        output += refName;
      } else {
        Params.push(Name);
...
function ParseParamObject(ParamObject, { scope, vars, varExistent, tabs, constants, $such, that, Insert, Params, comments = [], lastNodeLocation, registry = {}, type }) {
  var output = "{";
  for (var i = 0; i < ParamObject.length; i++) {
    ...
    if (Entry instanceof Nodes.ParamIdentifier) {
      output += ParseParam(new Nodes.Param("ParamIdentifier", Entry).setLoc(Entry), { scope, vars, varExistent, tabs, constants, $such, that, varExistent, Insert, Params, comments });
    } else {
      if (Expansion) output += "...";
      output += Key;
      if (!Value) Params.push(Key);
      if (Value) output += ": " + (registry.sources ? registry.sources.add(Value.loc) : '') + ParseParam(Value, { scope, vars, varExistent, tabs, constants, $such, that, varExistent, Insert, Params, comments });
      if (Defaults) output += " = " + Defaults.parse({ scope, vars, varExistent, tabs, constants, $such, that, varExistent, isValue: true, isParam: true, comments });
    }
    output += ","
  }
...
function ParseParamArray(ParamArray, { scope, vars, varExistent, tabs, constants, $such, that, Insert, Params, comments = [], lastNodeLocation, registry }) {
  var output = "[";
  for (let Entry of ParamArray) {
    if (Entry) {
      output += "" + ParseParam(Entry, { scope, vars, varExistent, tabs, constants, $such, that, varExistent, Insert, Params, comments, comments })
    }
    output += ", ";
  }
...
function loadSoaks(res, { tabs, scope, vars, prevLine, Soaks, registry, loc }) {
  ...
  for (let [soak] of Soaks) {
    ss += `${tab(tabs + i)}${registry.sources ? registry.sources.add(loc) : ''}if (${soak}) {\n`;
    i++;
  }
...
function addMetaDataToPrevLine({ typeData, prevLine, metaComments, tabs }) {
  let results = [];

  attachDescriptions(metaComments, typeData, tabs);
...
function buildVarDefinitionsWithJSDoc(typeData, tabs, keyword, metaComments) {
  let results = [];
...
function buildVarDefinitions(typeData, tabs, keyword, metaComments, rewrite, registry = {}) {
  ...
  for (let def of typeData) {
    ...
    if (build.length) {
      results.push(`/**\n${tab(tabs)} * ${build.join(`\n${tab(tabs)} * `)}\n${tab(tabs)} */\n${tab(tabs)}${keyword}${registry.sources ? registry.sources.add(def.loc) : ''}${def.ID}${def.value ? `: ${def.value}` : ''};`);
      rewrite.push(def.ID);
    }
  }

  return results.length ? tab(tabs) + results.join('\n\n' + tab(tabs)) + '\n\n' : '';
}

function attachDescriptions(metaComments, typeData, tabs) {
  let scaned = 0;
...
function getDataOfMetaComments({ metaComments, ID, tabs, loc }) {
  let result;
