...
Nodes.Class = class Class extends Base {
  parse({ that, $such, scope, vars = [], varExistent, constants, prevLine, isValue, tabs = 0, lineReturns, addSemicolon, comments = [], lastNodeLocation, registry = {} } = {}) {
    let output = "", className, classExtends, classAugment, puppet, nom;
    ...
    if (!ForeignExps.length) {
      ...
      if (ThisProperties.length && !this.isRecomp) {
        Body = ThisProperties.map((Type) => {
          return `${Type.unwrap.parse({ registry, addSemicolon: [] })}${Type.loc.type.optional ? '?' : ''}: ${Type.loc.type.value};\n${tab(registry.tabSize, tabs + 1)}`
        }).join('') + Body;
      }
      ...
      let classBlock = new Nodes.Line("Expression",
        new Nodes.Expression("Class",
          new Nodes.Class(null, [nom], Class[1], new Nodes.Body(null, Body), true, { ctor: classCtor, args: classArguments }).setLoc(this.loc)
        ).setLoc(this.loc)
      ).setLoc(this.loc);
...
Nodes.Function = class Function extends Base {
  parse({ that, $such, scope, vars, varExistent, constants, prevLine, isValue, tabs, func, scopedParams, comments, lastNodeLocation, registry = {}, FiresSuper, isInvoked, isParam, isClass, isObject, lineReturns, isExport, isLine, metaComments = [], metaQueue }) {
    ...
    for (let l = DocParams.length, i = 0; i < l; i++) {
      ...
      let [ID, loc, metaType] = Param;

      metaType = metaType ? { value: metaType.nodes.parse({ registry }), loc: metaType.nodes.loc, ID } : undefined;

      if (!metaType) {
        metaType = getDataOfMetaComments({ metaComments, ID, tabs, loc, registry, strict: true });
      } else {
    ...
    if (!!ParamData.find(d => d.description || d.value)) {
      if (!registry.isTypeScript && registry.omitTypeScript) {
        ParamData.map(d => {
          delete d.value;
        });
      ...
      buildFunctionDefinitionWithJSDoc({ ParamData, registry, tabs, loc: this.loc, prevLine, scope });
    }
...
Nodes.Interpolation = class Interpolation extends Base {
  parse(opts) {
    switch (this.rule) {
      case "String": {
        return this.unwrap.parse(opts).replace('`', '\\`');
      };
      default: {
  ...
  parse({ addSemicolon = [], construct, constants, vars, varExistent, that, $such, scope = [], isValue, isInvoked, isOperation, isLine, prevLine, tabs = 0, isCondition, isDirect, isClass, isCompare, isAssignment, ID, isParam, nl, func, scopedParams, comments = [], lastNodeLocation, registry = {}, isParenthetical, FiresSuper, FiresAwait, FiresYield, afterParse = [], metaComments = [] } = {}) {
    ...
    switch (this.rule) {
      case "Type": {
        ...
        let chunk = `type ${member} = ${sentences.parse({ registry })}`;
        ...
        statements.contents.forEach((statement, i, r) => {
          ...
          if (/boolean|undefined/.test(typeof self)) {
            ...
            if (self.loc.type && registry.isTypeScript && !registry.omitTypeScript) {
              ...
              var typeMatch = self.loc.type, prevl = [];

              addMetaDataToPrevLine({ typeData: [{ ID: self.parse({}), ...self.loc.type }], prevLine: prevl, metaComments, tabs, registry });

              prevLine.push(...prevl.map(l => untab(registry.tabSize, tabs, l, originalTabs)), `${keyword[0].toLowerCase() + ' '}${that};`);
            ...
            if (hasTypes.length && registry.omitTypeScript) {
              prevLine.push(...prevL.map(l => untab(registry.tabSize, tabs, l, originalTabs)), `${keyword[0].toLowerCase() + ' '}${them};`);


              Statements = Statements.replace(/,\s*(\n\s+)?$/, '') + (!isLast ? ", " : "");
              return;
            } else if (hasTypes.length) {
...
function ParseParam(Parameter, { scope, vars, tabs, constants, $such, that, varExistent, Insert, InsertAll, Params, comments = [], lastNodeLocation, registry = {}, i, isInside, isObject, isArray, Types, ValueKey, metaComments }) {
  ...
  switch (Parameter[0]) {
    ...
    case "ParamObject": {
      Params.push({ symbol: '{', label: ValueKey });
    ...
    if (Entry instanceof Nodes.ParamIdentifier) {
      ...
      if (Loc && Loc.type) {
        ...
        if (!registry.omitTypeScript) {
          Types.push((registry.sources ? registry.sources.add(Loc) : '') + Key + ': ' + (registry.sources ? registry.sources.add(Loc) : '') + 'any');
        }
      }
    }
    output += ","
...
function buildFunctionDefinitionWithJSDoc({ ParamData, registry, tabs, loc, prevLine, scope }) {
  var defs = [], refs = [];
  ...
  for (let data of ParamData) {
    if (data.symbol) {
      ...
      if (['{', '['].includes(data)) {
        ...
        defs.push(` * @param {${data === '[' ? 'Array<any>' : 'object'}} ${$refs(true)}${data === '[' ? ' -' : ''}`);
      } else {
        var last = refs.pop();
    ...
    if (!$isArrayRoot()) {
      ...
      line += `${$refs() ? $refs() + ($isArray() ? '' : '.') : ''}${!$isArray() ? data.ID : ''}`;
      ...
      if (name) {
        ...
        if (/^\s*\{/.test(comment.slice(i))) {
          ...
          let nodes = (() => {
            ...
            try {
              retv = Elysion.compile(`type PARTIAL = ${chunk}`, {
                cursor: {
                  x: fc - 13,
                  y: line
                },
...
function getDataOfMetaComments({ metaComments = [], ID, tabs, loc, registry, strict, noUpdate }) {
  let result;
