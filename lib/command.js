...
c = (f, s = '', _d, st, r, n, e) => {
  ...
  if (!st && fileIsDir(s ? path.join(s, f) : f)) {
    if (!fs.existsSync(file.replace(/(\\|\/)$/, '') + (file.endsWith('.els') ? '' : '.els'))) {
      (async function () {
  ...
  generatedFile = r ? f.replace(/\.els(on)?$/, r) : f + isELSON ? '.json' : '.js';

  var isTypeScript, compiledTS;

  ({ output, sourceMap, sources, isTypeScript, compiledTS } = Elysion.compile(content, { filename: f.split(/\\|\//g).pop(), generatedFile: n ? '<anonymous>' : generatedFile.split(/\/|\\/g).pop(), ...opts, dirname: path.join(cwd, s || ''), src: file, isELSON }));

  if (isTypeScript && typeof ts !== 'undefined') {
    generatedFile = generatedFile.replace(/.js$/, '.ts');
  } else if (isTypeScript) {
    if (compiledTS) output = compiledTS;
    isTypeScript = false;
  }
...
d = ([f, o, ...excludes], st, rename = ".els.js") => {
  ...
  folder = folder.filter(file => {
    ...
    let check = [excludes.findIndex(e => e === file) > -1, excludes.findIndex(e => e === file.replace(/\.els$/, '')) > -1].includes(true)
  ...
  for (let file of folder) {
    if (fileIsDir(f, file)) {
      ...
      let isELSON;
      if (!file.endsWith('.els') && !(isELSON = file.endsWith('.elson'))) continue;
...
cfg = () => {
  ...
  if (typeof JSON.parse(configData.output) !== 'object' || Array.isArray(JSON.parse(configData.output))) return process.stdout.write(red + 'elsconfig.json: ' + reset + 'config file content does not match with type ' + yellow + 'Object' + reset + '\n');
  ...
  let { outDir, outExtension } = compilerOptions, isFromSource;
  ...
  if (sourceDir !== undefined) {
    ...
    if (fileIsDir(sourceDir)) {
      files = fs.readdirSync(path.join(cwd, sourceDir), { encoding: 'utf-8' }).filter(f => (f.endsWith('.elson') || f.endsWith('.els') && !f.endsWith('.d.els')) || fileIsDir(sourceDir, f));
    } else {
  ...
  if (files !== undefined) {
    ...
    if (tsFiles.length) {
      ...
      let allDiagnostics = [], program = !opts.compilerOptions.omitDiagnostics && ts.createProgram(tsFiles, { ...opts.compilerOptions, emitDeclarationOnly: true, declaration: true, outDir: path.join(cwd, outDir), noImplicitAny: true });
      ...
      if (program) {
        ...
        allDiagnostics.forEach(({ file, start, length, code, category, messageText, relatedInformation }) => {
          if (file === undefined) {
            return;
          }
          ...
          if (location) {
            ...
            let reg = /^Type .(.+). is not assignable to type .(.+).\./.exec(messageText);
            ...
            try {
              ...
              throwSyntaxError({
                message: messageText.replace(/Type (.+) is not assignable to type (.+)\./, `Type ${yellow + '$1' + reset} is not assignable to type ${yellow + '$2' + reset}.`),
                location: {
