const { errorToString, red, updateSyntaxError, throwSyntaxError, resolvePath, ElysionError } = require('./helpers');
...
class Lexer {
  constructor(script, options = {}) {
    ...
    while (this.chunk = script.slice(i)) {
      ...
      if (!consumed) {
        ...
        throwSyntaxError({
          message: msg,
          location: { first_line: this.cursor.y, first_column: this.cursor.x, last_column: this.cursor.x + 1, last_line: this.cursor.y, src: this.filename ? require('path').join(this.sdir, this.filename) : '<anonymous>' },
          type: 'SyntaxError'
        });
      }
    ...
    if (typeof require === "function" && !options.isDefinitions && this.filename && this.filename !== "<anonymous>") {
      let fs = require('fs'), path = require('path');
      let deff = this.filename.split('.').slice(0, -1).join('.') + '.d.els';
      let deft = path.join(this.dirname, deff);

      if (fs.existsSync(deft)) {
        let { comments } = new Lexer(fs.readFileSync(deft, { encoding: 'utf-8' }), {
          filename: deff,
    ...
    if ((process.argv.includes("-t") || options.streamTokens) && !this.isInterpolation) console.log(this.tokens);
  }
  ...
  Type() {
    ...
    if (ref = /^<([:?])/.exec(this.chunk)) {
      ...
      optional = optional === '?';
      let nodes = (() => {
        let Elysion = require('../index'), retv;
      ...
      if (!nodes) {
        throwSyntaxError({
          ...
          location: {
            ...
            src: this.filename ? require('path').join(this.sdir, this.filename) : '<anonymous>'
          },
          type: 'SyntaxError'
        }, this.code.split('\n')[this.cursor.y - 1]);
      }
      ...
      while (++i) {
        ...
        if (/^[)\]}>]/.test(char)) {
          ...
          if (symbols.pop() !== closure) {
            ...
            throwSyntaxError({
              message: msg,
              location: { first_line: virtualLoc.y, first_column: virtualLoc.x, last_column: virtualLoc.x + 1, last_line: virtualLoc.y, src: this.filename ? require('path').join(this.sdir, this.filename) : '<anonymous>' },
              type: 'SyntaxError'
            }, this.code.split("\n")[virtualLoc.y - 1]);
          }
        }
    ...
    if (ref = reg.exec(this.chunk)) {
      ...
      if (!this.isELSON && (/\{|\[|\.(\.\.)?/.test(token) && (token === "." && !this.prev()[0] || (["IDENTIFIER", "PROPERTY", "SYMBOL_EXISTS"].includes(this.prev()[0]) && this.prev().spaced && !(/^\s*(\n|;)/.test(this.chunk.slice(token.length)) && this.opLine)) || this.prev()[0] && !["IDENTIFIER", "PROPERTY", "]", "INDEX_END", "STRING_END", "STRING", "REGEX", "REGEX_END", ".", ")", "CALL_END", "}", "THIS", "SUCH", "SUPER", "OUTDENT"].includes(this.prev()[0])))) {
        if (this.prev() && this.prev()[0] && "IDENTIFIER SUCH SUPER PROPERTY ] INDEX_END ) CALL_END } SYMBOL_EXISTS".split(' ').includes(this.prev()[0])) {
          let b;
          if ((this.prev()[0] === "IDENTIFIER") && (this.inClass() && !(b = this.skipped === this.indentLevel) || this.assertTokens("CLASS IDENTIFIER", "CLASS IDENTIFIER <( .+ )>"))) {
            this.token(
              'PARAM_START',
              '(',
      ...
      } else if (token === ":" && this.assertTokens('CLASS IDENTIFIER <( .+ )>')) {
      ...
      let x;
      if (this.explicit.find(explicit => explicit[0] === ")>") || this.stage().typeKwd || this.funcLine === this.cursor.y || this.assertTokens('CLASS', 'CLASS IDENTIFIER', 'CLASS IDENTIFIER WITH') || (x = this.prev()[0] === "WITH" || this.prev()[0] === "WITHIN")) {
        Open.push('<');
        Close.push('>');
      }
      ...
      if ((_ref = Open.indexOf(token)) > -1) {
        if ((token === "(") && !([...this.Opening, "NEWLINE"].includes(prev[0])) && (!prev || ["FUNCTION", "WITH", "WITHIN"].includes(prev[0]) || this.assertTokens('CLASS IDENTIFIER') || !prev.spaced)) {
          let z;
        ...
        if ((token !== "PARAM_START" && token !== "<(") && (this.prev()[0] === "WITH" || this.prev()[0] === "WITHIN" || this.prev()[0] === ")>" && this.funcLine === this.cursor.y && !this.stages.find(s => s.contains.find(c => c.includes('param'))))) {
          ...
          this.paramLine = true;
          this.prev().stageId = this.store(['param', this.position]);
        } else if (((token !== "(") && token !== "<(") && (!this.opLine) && "SUCH SUPER IDENTIFIER PROPERTY ] INDEX_END CALL_END ) } SYMBOL_EXISTS".split(' ').includes(this.prev()[0]) && this.prev().spaced && !this.paramLine) {
  ...
  Identifier() {
    ...
    if (reg.test(this.chunk) || (numReg.test(this.chunk) && (isNumber = true))) {
      ...
      if (prev && prev[0] && prev.spaced && "SUCH SUPER IDENTIFIER PROPERTY ] CALL_END ) } SYMBOL_EXISTS )>".split(' ').includes(prev[0])) {
        ...
        let b;
        if ((prev[0] === "IDENTIFIER" || prev[0] === ")>") && this.inClass() && !(b = this.skipped === this.indentLevel) || (this.funcLine === this.cursor.y) || this.assertTokens("CLASS IDENTIFIER", "CLASS IDENTIFIER <( .+ )>")) {
          this.token(
            'PARAM_START',
            '(',
            { generated: true, pair: this.position }
      ...
      if (((tag == "IDENTIFIER") || (tag == "VARIDENTIFIER") || (tag == "THIS") || (tag == "SUCH") || (tag == "SUPER") || this.assertTokens('THIS . PROPERTY') && this.tokens.slice(-3)[0][2].generated && this.paramLine) && colon && (this.inExplicit() && (/^(A[ST]|FROM|:|\[|\(|PARAM_START|DEFAULTS)$/.test(prev[0])) || (!this.inExplicit() || this.currExplicit() !== "}") && (this.currExplicit() !== "}" || this.stage().label !== undefined)) && !b) {
        if (!this.stage().contains.find(o => o[0] === "object")) {
          ...
          if ((this.prev()[0] == "NEWLINE") && (this.prev()[1] === 0) && (this.prev().isNext2GenObj || (this.tokens.slice(-2)[0] || [])[0] === "}")) {
            ...
            } else if (this.assertTokens('THIS .')) {
  ...
  KeywordStatement() {
    ...
    if (reg.test(this.chunk) || !this.isELSON && ((reg = /^(@)\s*/).test(this.chunk) || (reg = /^(with(?:in)?(?=[([{<]))/).test(this.chunk)) || (reg = /^(=(?!(?:>|=)))/).test(this.chunk)) {
      ...
      let label;
      if (token === "@" && (label = /^((?:[^\x00-\x7F]|[a-z$_])(?:[^\x00-\x7F]|[a-z$_\d])*)(?::(?!:))/.exec(this.chunk.slice(input.length)))) {
        this.token('IDENTIFIER', label[1], {
          loc: {
            first_column: this.cursor.x,
            first_line: this.cursor.y,
            last_line: this.cursor.y,
            last_column: this.cursor.x + label[1].length
          }
        });
        this.token('::', ':', {
          loc: {
            first_column: this.cursor.x + label[1].length,
            first_line: this.cursor.y,
            last_line: this.cursor.y,
            last_column: this.cursor.x + label[1].length + 1
          }
        });

        return input.length + label[0].length;
      }
  ...
  Separator() {
    ...
    if (reg.test(this.chunk)) {
      ...
      if (this.trueStage().assignKwd && aligned.length === (this.trueStage().assignKwd.length + 1)) {
      ...
      if (indent == this.indentLevel) {
        ...
        if (this.actExp && (this.prev()[0] !== "OUTDENT") && (this.prev()[0] !== "NEWLINE") && !bool && (this.prev()[0] !== "IF") && (this.prev()[0] !== "SWITCH") && (this.prev()[0] !== "WHILE") && (this.prev()[0] !== "[") && (this.prev()[0] !== "{") && (this.prev()[0] !== "(" && (this.prev()[0] !== ":") && !/^(A[ST]|FROM)$/.test(this.prev()[0]))) {
          ...
          if ([inArrayLevel, inAssignChain].includes(true)) {
            ...
            if (this.prev()[0] !== "," && !["CONST", "VAR", "LET"].includes(this.prev()[0])) {
  ...
  importPath() {
    ...
    if (reg.test(this.chunk) && this.prev()[0] === "IMPORT" && this.prev().spaced) {
      ...
      if (typeof require !== 'function') {
        throwSyntaxError({
          message: 'Unable to load imports in browser compiler',
          location: { first_line: this.cursor.y, first_column: this.cursor.x, last_column: this.cursor.x + match[0].length, last_line: this.cursor.y, src: joinPath(this.sdir, this.filename) },
          type: 'error'
        }, this.code.split("\n")[this.cursor.y - 1]);
      }

      let file = resolvePath(match[0], this.dirname), dirname = require('path').dirname(file);

      if (fileIsDir(file)) {
        file = require('path').join(file, 'index.els');
      }
      ...
      if (!require('fs').existsSync(file)) {
        ...
        throwSyntaxError({
          message: msg,
          location: { first_line: this.cursor.y, first_column: this.cursor.x, last_column: this.cursor.x + match[0].length, last_line: this.cursor.y, src: require('path').join(this.sdir, this.filename) },
          type: 'error'
        }, this.code.split("\n")[this.cursor.y - 1]);
      } else if (this.cantImport.includes(require('path').relative(file, this.sdir))) {
      ...
      let source = require('fs').readFileSync(file, { encoding: 'utf-8' });

      let Include = new Lexer(source, {
        isInclude: true,
        filename: require('path').relative(this.sdir, file),
        sdir: this.dirname,
        dirname,
        cantImport: this.cantImport.concat(require('path').relative(file, this.sdir))
      });
  ...
  String() {
    ...
    if (match) {
      ...
      switch (fragments.length) {
        ...
        case 1: {
          if (this.prev()[0] === "IMPORT") {
            let file = resolvePath(fragments[0][1], this.dirname), dirname = require('path').dirname(file);

            if (!require('fs').existsSync(file)) {
              let msg = "file not found";
              throwSyntaxError({
                message: msg,
                location: { first_line: this.cursor.y, first_column: this.cursor.x, last_column: this.cursor.x + fragments[0][1].length + 2, last_line: this.cursor.y, src: require('path').join(this.sdir, this.filename) },
                type: 'error'
              }, this.code.split("\n")[this.cursor.y - 1]);
            } else if (this.cantImport.includes(require('path').relative(file, this.sdir))) {
              let msg = "prevented endless loop from import";
  ...
  isPossibleArray() {
    return (this.stage().type === "indent") && (this.stage().label !== "import") && (this.stage().label !== "export") && !this.stage().contains.length && !this.stage().assignKwd && (this.portLine !== this.cursor.y && this.forAssignLine !== this.cursor.y) && (!this.inExplicit() && !(this.currExplicit() && !this.paramLine && [undefined, 'Root'].includes(this.stage().label)))
  }
  ...
  assertTokens(...str) {
    if (str.length > 1) {
      ...
      for (let query of str) {
        checks.push(this.assertTokens(query));
      }
...
function fileIsDir() {
  let subfolder;
  try {
    subfolder = require("fs").readdirSync(require("path").join.apply(null, arguments));
  } catch (_) { }
