 ...
 */
 ...
 const { Parser, Nodes } = require('./parser'),
 Lexer = require('./lexer');

const { parser } = require("./elysion.js"),
 { errorToString, updateSyntaxError, guessIndentation, ElysionError, red, throwSyntaxError } = require("./helpers"),
 { SourceMap } = require('./sourcemaps');
...
function prettyPrint(cb) {
 return function (script, options) {
   try {
     ...
     if (err.token && (ref = err.token[2].src) !== options.src && ref || ((ref = err.src) !== options.src) && ref) {
       options.src = ref;
     }
